-- Non-destructive additive migration (production PostgreSQL) — STAGE 12 versioned pay
-- schemes. New nullable/defaulted columns + unique index on sourceChangeRequestId. No
-- ALTER TYPE, no table rebuild, no recompute. Existing rows read as version=1,
-- status='active'; backfill (payroll:scheme-backfill) refines status by dates.
...
ALTER TABLE "EmployeePayScheme" ADD COLUMN "submittedAt" TIMESTAMP(3);
ALTER TABLE "EmployeePayScheme" ADD COLUMN "approvedById" TEXT;
ALTER TABLE "EmployeePayScheme" ADD COLUMN "approvedAt" TIMESTAMP(3);
ALTER TABLE "EmployeePayScheme" ADD COLUMN "activatedAt" TIMESTAMP(3);
ALTER TABLE "EmployeePayScheme" ADD COLUMN "archivedAt" TIMESTAMP(3);
