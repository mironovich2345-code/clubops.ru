...
async function main() {
  ...
  check("OFD-COMPACT1 основные блоки сгруппированы; технические диагностики свернуты под Collapsible", pageSrc.includes("function Collapsible") && advIdx > 0 && pageSrc.includes("Технический блок для проверки ответов Такском"));
  check("OFD-COMPACT2 История синхронизаций свернута по умолчанию (Collapsible, без open)", pageSrc.includes('<Collapsible title="История синхронизаций"') && !/<details[^>]*\bopen\b/.test(pageSrc));
  ...
  check("OFD-COMPACT4 обычные блоки доступны: Подключения/Кассы/Синхронизация/ОФД продажи/Статьи доходов + ссылка на аналитику", pageSrc.includes('<Section title="Подключения Такском">') && pageSrc.includes('<Section title="Кассы ККТ">') && pageSrc.includes('<Section title="Синхронизация продаж">') && pageSrc.includes('<Section title="ОФД продажи">') && pageSrc.includes('<Section title="Статьи доходов ОФД">') && pageSrc.includes('href="/analytics/ofd-sales"'));
