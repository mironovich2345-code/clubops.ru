...
async function main() {
  ...
  check("ACT-MULTI actions: saveOfdConnection multi (editId), addOfdMapping registerKind + legalEntity-from-connection, toggleOfdConnection", actions.includes("const editId = str(formData,") && actions.includes('registerKind = str(formData, "registerKind") === "online_cashbox"') && actions.includes("legalEntityId: connection.legalEntityId") && actions.includes("export async function toggleOfdConnection"));
  ...
  check("DASH-OFD5 owner/GD видит короткий dashboard по всему scope (ключевые карточки из ОФД по всем клубам)", dScopeAll.byClub.length === 2 && dashPageSrc.includes("<DashboardKeyCards") && dashPageSrc.includes("cards.reduce((a, c) => a + c.ofd.ofdSubscriptionsKopeks"));
  ...
  const keyCardsComp = readFileSync(new URL("../src/app/(app)/dashboard/_components/DashboardKeyCards.tsx", import.meta.url), "utf8");
  ...
  check("DASH-USE-OFD1 dashboard использует ОФД (ключевые карточки), не показывает 0 при наличии выручки", mgmt.hasData === true && mgmt.perClub.get("clubA").incomeKopeks === 600000 && mgmt.perClub.get("clubB").incomeKopeks === 300000 && dashPage.includes("<DashboardKeyCards") && dashPage.includes("loadOfdManagementOverview(companyId, clubIds, period.start, period.end)"));
  check("DASH-USE-OFD2 «Абонементы» берутся из ОФД-категории membership", mgmt.perClub.get("clubA").subscriptionsKopeks === 400000 && mgmt.totals.subscriptionsKopeks === 580000 && dashPage.includes("const keyAbKopeks = useOfd ? ofdMgmt!.totals.subscriptionsKopeks"));
  check("DASH-USE-OFD3 «ПТ» берётся из ОФД-категории personal_training", mgmt.perClub.get("clubA").personalTrainingKopeks === 150000 && dashPage.includes("const keyPtKopeks = useOfd ? ofdMgmt!.totals.personalTrainingKopeks"));
  ...
  check("DASHBOARD-CARD1 dashboard показывает ОФД-выручку при наличии OfdDailySalesSummary (ключевые карточки)", dashPage.includes("<DashboardKeyCards") && dashPage.includes("loadOfdManagementOverview(companyId, clubIds, period.start, period.end)") && keyCardsComp.includes('label="Абонементы"'));
  check("DASHBOARD-CARD2 Абонементы/ПТ dashboard берутся из ОФД membership/PT (totals)", dashPage.includes("const keyAbKopeks = useOfd ? ofdMgmt!.totals.subscriptionsKopeks") && dashPage.includes("const keyPtKopeks = useOfd ? ofdMgmt!.totals.personalTrainingKopeks"));
  check("DASHBOARD-CARD3 результат dashboard = ОФД net − confirmed costs", dashPage.includes("computeManagementResult(ofdMgmt?.totals.netKopeks ?? 0, s.expensesKopeks)"));
  ...
  check("DASH-SIMPLE3 dashboard показывает только ключевые карточки (Абонементы/ПТ/Расходы/Наличные ООО/ИП/Результат)", dashPage.includes("<DashboardKeyCards") && keyCardsComp.includes('label="Абонементы"') && keyCardsComp.includes('label="ПТ"') && keyCardsComp.includes('label="Фактические расходы"') && keyCardsComp.includes('label="Наличные ООО"') && keyCardsComp.includes('label="Наличные ИП"') && keyCardsComp.includes('label="Результат"'));
  check("DASH-SIMPLE4 owner/GD видят короткий dashboard (стратегическая ветка → DashboardKeyCards, без сетки клубных карточек)", /return \(\s*<div className="mx-auto max-w-\[1440px\]">[\s\S]*?<DashboardKeyCards/.test(dashPage) && dashPage.includes("cards.reduce((a, c) => a + c.ofd.ofdSubscriptionsKopeks"));
  check("DASH-SIMPLE5 regional видит короткий dashboard только по allowedClubIds (scoped clubIds)", dashPage.includes("const clubs = await getUserClubs(user.id, companyId)") && dashPage.includes("loadOfdManagementOverview(companyId, clubIds, period.start, period.end)") && dashPage.includes("loadScopeCashFactTotals(companyId, clubIds, now)"));
  ...
  check("DARK-CARDS1 карточки Dashboard без выбивающихся светлых фонов в dark mode (нейтральные токены)", keyCardsComp.includes("dark:bg-slate-900") && keyCardsComp.includes("dark:border-slate-800") && !/bg-(emerald|rose|amber|brand|sky)-50/.test(keyCardsComp) && !keyCardsComp.includes("bg-brand-50"));
  ...
  check("AUDIT-DOUBLECOUNT1 ОФД возврат уменьшает net; paid refund в расходах; НЕТ автосвязи (импорт не создаёт refund/expense) — риск задокументирован", !importerSrc.includes("prisma.refund.create") && !importerSrc.includes("prisma.expense.create") && analyticsLibSrc.includes('...data.refunds.map((r) => ({ clubId: r.clubId, category: "refunds"') && mgmtLib.includes("returnsKopeks") && auditDoc.includes("Returns double-counting") && auditDoc.includes("RISK-1"));
  ...
  check("AUDIT-PAGE5 Dashboard не использует SalesReport для текущей выручки (ОФД первично, report fallback)", dashPage.includes("const useOfd = !!ofdMgmt && ofdMgmt.hasData") && dashPage.includes("const keyAbKopeks = useOfd ? ofdMgmt!.totals.subscriptionsKopeks") && !dashPage.includes("salesSplitTrend"));
  ...
  check("AUDIT-ROLE4 regional ограничен allowedClubIds", analyticsPage.includes("const aClubIds = groups ? groups.filteredClubIds : ctx.allowedClubIds") && dashPage.includes("const clubs = await getUserClubs(user.id, companyId)") && mgmtLib.includes("clubId: { in: clubIds }"));
