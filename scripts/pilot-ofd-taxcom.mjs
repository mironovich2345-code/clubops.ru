...
async function main() {
  ...
  check("OFD-COMPACT10 после раскрытия «Кассы ККТ» форма OfdMappingForm и менеджер касс (карточки) доступны внутри блока", kassyIdx > 0 && pageSrc.indexOf("<OfdMappingForm") > kassyIdx && pageSrc.indexOf("<OfdMappingForm") < syncIdx && pageSrc.indexOf("<CashRegisterManager") > kassyIdx && pageSrc.indexOf("<CashRegisterManager") < syncIdx);
  ...
  check("MULTI-OFD7 online_cashbox renders as 'Онлайн-касса' (and club_cashbox as 'Касса клуба') in the cash-register cards", (() => { const mgr = readFileSync(new URL("../src/app/(app)/settings/integrations/ofd/_components/CashRegisterManager.tsx", import.meta.url), "utf8"); return mgr.includes('online_cashbox: "Онлайн-касса"') && mgr.includes('club_cashbox: "Касса клуба"') && mgr.includes("KIND_LABEL[r.registerKind]"); })());
  ...
  check("UI-MULTI2 'Кассы ККТ' — менеджер касс (карточки) показывает ФН/РНМ/подключение/юрлицо/клуб/тип/статус + действия", (() => { const mgr = readFileSync(new URL("../src/app/(app)/settings/integrations/ofd/_components/CashRegisterManager.tsx", import.meta.url), "utf8"); return pageSrc.includes('title="Кассы ККТ"') && pageSrc.includes("<CashRegisterManager") && mgr.includes("ФН ") && mgr.includes("РНМ") && mgr.includes("connectionName") && mgr.includes("legalName") && mgr.includes("clubName") && mgr.includes("registerKind") && mgr.includes("STATUS[r.status]") && mgr.includes("Изменить") && mgr.includes("Удалить"); })());
  ...
  // REM-05A: official profit = calculateProfit (ОФД net − признанные расходы), not ofdNet−confirmedCosts.
  check("ANALYTICS-USE-OFD6 результат = calculateProfit (ОФД net − признанные расходы)", analyticsPage.includes("calculateProfit(") && analyticsPage.includes("profitResult.profitKopeks"));
  ...
  check("ANALYTICS-CARD4 «Прибыль» карточки = calculateProfit (ОФД net − признанные расходы)", analyticsPage.includes("profit={profitResult.profitKopeks}") && analyticsPage.includes("Признанные расходы"));
  ...
  check("DASHBOARD-CARD3 результат карточки = ОФД net − признанные расходы (REM-05A)", keyCardsComp.includes("card.ofd.ofdNetKopeks - card.recognizedExpensesKopeks"));
  ...
  check("AUDIT-FORMULA10 IP expense pending влияет на cash, но не на profit (cash-balances pending sourced from expense-status; analytics confirmed/verified)", (() => { const statusSrc = readFileSync(new URL("../src/lib/expense-status.ts", import.meta.url), "utf8"); return cashBalancesSrc.includes("IP_EXPENSE_PENDING_STATUSES: readonly string[] = EXPENSE_CASH_PENDING_STATUSES") && /EXPENSE_REVIEW_STATUSES[\s\S]*?"submitted"/.test(statusSrc) && statusSrc.includes('EXPENSE_VERIFIED_STATUSES = ["verified", "confirmed"]') && !/EXPENSE_REALIZED_STATUSES\s*=\s*\[[^\]]*submitted/.test(budgetsSrc); })());
  ...
  check("AUDIT-PAGE6 Analytics не использует SalesReport для текущей выручки/результата (ОФД первично)", analyticsPage.includes("const abValue = useOfd ? ofd!.totals.subscriptionsKopeks") && analyticsPage.includes("profit={profitResult.profitKopeks}") && !analyticsPage.includes("oooKopeks={s.cashOooRemainingKopeks}"));
  ...
  check("AUDIT-BUGFIX1 Analytics «Финансовый итог» = calculateProfit (ОФД net − признанные расходы) + факт-остаток из cash-balances", analyticsPage.includes("profit={profitResult.profitKopeks}") && analyticsPage.includes("loadScopeCashFactTotals(aCompanyId, aClubIds, now)") && analyticsPage.includes("Выручка по ОФД") && auditDoc.includes("BUG-1"));
  ...
  check("AUDIT-REFUND-POLICY2 official profit = calculateProfit; legacy mgmtResult deprecated (0 live callers)", mgmtLib.includes("return ofdNetKopeks - confirmedCostsKopeks") && mgmtLib.includes("@deprecated REM-05A") && analyticsPage.includes("calculateProfit(") && !analyticsPage.includes("computeManagementResult") && !keyCardsComp.includes("computeManagementResult"));
  ...
  check("BALANCES-LEGACY5 история контрольных остатков в /collections не сломана (теперь версионный timeline)", collPageSrc.includes("getSnapshotTimeline") && collPageSrc.includes("Хронология контрольных точек"));
  ...
  check("DASH-CLUB-CARDS10 Result = ОФД net − признанные расходы (REM-05A)", keyCardsComp.includes("card.ofd.ofdNetKopeks - card.recognizedExpensesKopeks") && mgmtResult(mgmt.totals.netKopeks, 100000) === mgmt.totals.netKopeks - 100000);
