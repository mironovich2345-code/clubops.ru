...
async function main() {
  ...
  check("CASHIER-DIAG2 cashier ФИО/ПДн never appear in the shape (only the presence boolean); nothing stored", (() => { const s = inspectDIShape({ document: { "1021": "Иванова Анна Петровна", "1203": "500100732259" } }); return s.hasCashierLikeKey === true && !/Иванова|Анна|Петровна|500100732259/.test(JSON.stringify(s)); })());
  ...
  check("CASHIER-DIAG-S adapter detects cashier KEY NAMES only (1021/1203), never the value; no cashier storage in OFD models", adapter.includes("hasCashierLikeKey") && adapter.includes('"1021"') && adapter.includes('"1203"') && adapter.includes("KEY NAMES only") && !/cashier/i.test(schema.slice(schema.indexOf("model OfdReceiptItem"), schema.indexOf("model OfdReceiptItem") + 900)) && !/cashier/i.test(schema.slice(schema.indexOf("model OfdReceiptImport"), schema.indexOf("model OfdReceiptImport") + 900)));
