...
function main() {
  ...
  check("BUDGET-IMPORT6 manager/regional/accountant/marketer не могут импортировать бюджеты (owner/GD only)", budgetActions.includes("if (!canImportPlansAndBudgets(ctx.effectiveRoles))") && /previewBudgetImport[\s\S]{0,400}canImportPlansAndBudgets/.test(budgetActions) && /applyBudgetImport[\s\S]{0,400}canImportPlansAndBudgets/.test(budgetActions) && budgetActions.includes("CATEGORY_KEYS.has(r.category)"));
