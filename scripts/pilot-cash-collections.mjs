...
async function main() {
  ...
  const keyCardsSrc = rd("src/app/(app)/dashboard/_components/ClubCard.tsx");
  const dashCardsLib = rd("src/lib/dashboard-cards.ts");
  ...
  check("OTHER-INCOME-CREATE create: canCreateOperational, комментарий обязателен, БЕЗ документов (documents: 0), pending_review, источник валидируется", actions.includes("export async function createCashOtherIncome") && actions.includes("canCreateOperational(g.roles)") && /if \(!comment\) return \{ ok: false, error: "Комментарий обязателен/.test(actions) && actions.includes("documents: 0") && !/createCashOtherIncome[\s\S]*?collectDocuments/.test(actions) && actions.includes('status: "pending_review"') && actions.includes("OTHER_INCOME_SOURCES"));
  ...
  const collComp = rd("src/app/(app)/collections/_components/SingleAccordion.tsx");
  check("COLLECTIONS-COMPACT1 видимы по умолчанию: «Фактические деньги» (recon+синхронизация) + карточки ООО/ИП (Section, без раскрытия)", pageSrc.includes('title="Фактические деньги — сверка наличных"') && pageSrc.includes("<CashSyncButtons") && pageSrc.includes("Остатки сейчас") && pageSrc.includes("<OooCard") && pageSrc.includes("<IpCard"));
  check("COLLECTIONS-COMPACT2 формы (Контрольный остаток/Инкассировать/Изъять/Пополнить ИП) под раскрывающимися блоками, свернуты по умолчанию", pageSrc.includes('id="control"') && pageSrc.includes('id="collect"') && pageSrc.includes('id="withdraw"') && pageSrc.includes('id="other"') && pageSrc.includes("<AccordionGroup>") && !/defaultOpenId=/.test(pageSrc));
  check("COLLECTIONS-COMPACT3 единая история операций за месяц (MonthNav + фильтры), под карточками", pageSrc.includes("История операций · ") && pageSrc.includes("<MonthNav") && pageSrc.includes("loadCollectionsHistory"));
  check("COLLECTIONS-COMPACT4 single-active accordion (открытие закрывает прочие; безопасно для server actions); формы внутри", collComp.includes("setOpenId(open ? null : id)") && collComp.includes("{children}") && pageSrc.includes("<CollectionForm") && pageSrc.includes("<OtherIncomeForm") && pageSrc.includes("<AccordionGroup>"));
  check("OTHER-INCOME-DOCS1b форма «Приход Иное»: поле документов ПОЛНОСТЬЮ убрано (нет MobileFileField/name=documents)", !oiFormBlock.includes('name="documents"') && !oiFormBlock.includes("MobileFileField") && oiFormBlock.includes("не требует подтверждающих документов"));
  check("OTHER-INCOME-DOCS2 исторические документ-строки остаются read-only (модель/лимит сохранены, действие ничего не удаляет)", docStore.includes("MAX_CASH_OPERATION_DOCUMENTS = 3") && !actions.includes("cashOperationDocument.delete"));
  ...
  check("DARK-COLLAPSIBLE1 раскрывающиеся блоки читаемы в тёмной теме (нейтральные токен-классы + .dark remap; не белые на тёмном)", collComp.includes("bg-white") && collComp.includes("border-slate-200") && collComp.includes("text-slate-800") && globalsCss.includes(".dark .bg-white") && globalsCss.includes(".dark .border-slate-200") && globalsCss.includes(".dark .text-slate-700"));
  ...
  check("CANCEL-UI история показывает кнопку «Отменить» только для отменяемых операций (draft/pending), для approved/rejected/cancelled — нет", forms.includes("export function CancelButton") && forms.includes(">Отменить<") && pageSrc.includes('(CANCELABLE[r.kind] ?? []).includes(r.status)') && pageSrc.includes("<CancelButton"));
  check("HISTORY-STATUSES единая история не скрывает cancelled (loadCollectionsHistory без status-фильтра → все статусы)", pageSrc.includes("loadCollectionsHistory") && !/cashCollection\.findMany\(\{ where: \{ companyId, clubId: \{ in: clubIds \}, status/.test(readFileSync(new URL("../src/lib/collections-history.ts", import.meta.url), "utf8")) && pageSrc.includes("STATUS_LABELS"));
  ...
  check("OPENING-S setCashOpeningBalance создаёт НОВУЮ контрольную точку (balanceSnapshot.create), не редактирует историю; корректировка — только флип на superseded (append-only); аудит cash.opening_balance_set", (() => { const setBlock = actions.slice(actions.indexOf("export async function setCashOpeningBalance"), actions.indexOf("export async function correctBalanceSnapshot")); return setBlock.includes("prisma.balanceSnapshot.create") && !/balanceSnapshot\.update/.test(setBlock) && actions.includes('"cash.opening_balance_set"') && actions.includes('data: { status: "superseded" }') && !/balanceSnapshot\.delete/.test(actions); })());
  ...
  check("DASHBOARD-CASH1 dashboard club card shows Наличные ООО/ИП from fact balance (loadClubCashBalances), gated by canSeeCash", keyCardsSrc.includes('label="Наличные ООО"') && keyCardsSrc.includes('label="Наличные ИП"') && rd("src/app/(app)/dashboard/page.tsx").includes("showCash={canSeeCash}") && dashCardsLib.includes("loadClubCashBalances") && dashCardsLib.includes("cashOooFactBalance") && dashCardsLib.includes("showCash: boolean") && loader.includes("export async function loadScopeCashFactTotals") && loader.includes("cashOooFactBalance"));
  check("DASHBOARD-CASH2 manager sees cash on club card but no pending/owner analytics (showCash gate)", keyCardsSrc.includes("showCash") && !keyCardsSrc.includes("Инкассации на проверке") && dashCardsLib.includes("cashIpFactBalance") && loader.includes("cashIpFactBalance"));
