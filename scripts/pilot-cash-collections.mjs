...
async function main() {
  ...
  check("OTHER-INCOME-DOCS1b форма «Пополнить ИП»: документы НЕ обязательны (MobileFileField без required), подпись «необязательно»", /name="documents" maxFiles=\{3\} \/>/.test(oiFormBlock) && !/name="documents" maxFiles=\{3\} required/.test(oiFormBlock) && oiFormBlock.includes("необязательно") && actions.includes("collectDocuments(formData, 0)"));
  check("OTHER-INCOME-DOCS2 «Приход Иное»: 1–3 документа допустимы (MobileFileField maxFiles=3 + общий max 3)", /name="documents" maxFiles=\{3\}/.test(oiFormBlock) && docStore.includes("MAX_CASH_OPERATION_DOCUMENTS = 3"));
