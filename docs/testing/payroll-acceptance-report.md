...
> ## ⟩ ОБНОВЛЕНИЕ 2026-07-24 (устранение блокеров) — ВЕРДИКТ: **READY FOR PILOT**
>
> Все 5 блокеров из первичной приёмки устранены (ветка `feat/payroll-blockers`), каждый —
> отдельным коммитом с полным гейтом (prisma validate dev+prod, tsc, build, build:prod).
> `npm run pilot:full` = **2766/0, 50 наборов**; prod-миграции без DROP/rebuild.
>
> 1. **Тренер ТЗ §4.2 — ПОЛНЫЙ E2E.** Схема `gym_trainer` подключена к `computeScheme`;
>    модель `PayrollTrainerPackage` + UI ввода пакетов/проведённых/возвратов/ставки; сводка
>    (продано/начисление/проведено/допустимо/кредит/долг); кредит и порог 70% РАЗДЕЛЬНЫ;
>    увольнение → удержание кредита (`trainer_credit_recovery`) → переплата = долг сотрудника,
>    не списывается. Тесты: `pilot-payroll-trainer` (18).
> 2. **Зарплата как фактический расход — БЕЗ двойного списания.** PayrollPayment/аванс →
>    ОДНА `Expense{category:"salary"}` (P&L + ООО/ИП фактбаланс) + ОДНО движение кассы
>    (`recordExpenseMovement`); отдельного payroll-CashMovement нет; сторно возвращает кассу
>    и P&L. Тесты: `pilot-payroll-finance` (13).
> 3. **Аванс до утверждения периода** — в открытом текущем месяце; ручная база earnedToDate
>    (комментарий + подтверждение регионала); авто-подхват периодом без повторного расхода.
>    Тесты: `pilot-payroll-preadvance` (14).
> 4. **Регионал — единый расчёт по городу** (одно начисление/город/месяц; revenue|profit;
>    утверждает собственник; выплаты частями из клубов; запрет переплаты без явного долга).
>    Тесты: `pilot-payroll-regional` (13).
> 5. **Выбор юрлица на уровне выплаты** (нал → ИП; безнал → ООО/ИП; проверка принадлежности
>    клубу; агрегация не двоится). Тесты: `pilot-payroll-legalentity` (11).
>
> **Честные оговорки (не блокеры пилота):** живой прогон против БД и браузерный smoke-test в
> этой среде НЕ выполнялись (нет сессии/браузера) — сценарии подтверждены прогоном pilot-набора
> + трассировкой кода; их живое прохождение и есть пилот (чек-лист — в §UI-SMOKE ниже).
> Streak-бонусы менеджера §4.5B по-прежнему ручная корректировка (вне объёма этих задач).
> Обязательство по переплате регионала, когда у регионала нет записи `ClubEmployee`,
> хранится с суррогатным `employeeId` (= id городского расчёта).
>
> Первичная приёмка (ниже) сохранена как история; её §0-вердикт «READY WITH LIMITATIONS»
> относился к состоянию ДО устранения блокеров.

---

# (Первичная приёмка — история)
...
# UI-SMOKE — ручной чек-лист (обновление блокеров)

Браузерный прогон в этой среде **не выполнялся** (нет браузера). Ниже — точный чек-лист для
ручной проверки на пилоте; каждая позиция уже собрана в `next build`/`build:prod` (маршруты
скомпилированы) и покрыта pilot-набором.

**Тренер ТЗ (карточка тренера):** `/payroll` → сотруднику задать схему «Тренер ТЗ (пакеты
40/50%)» → `/payroll/periods` создать период → «Сформировать расчёты» → в карточке тренера:
- добавить пакет 20 000 ₽ / 10 занятий → начисление 8 000 (40%);
- добавить пакет 30 000 ₽ → 50%;
- ввести проведено 5 из 10 → «допустимо к выплате» 4 000, «кредит тренера» 4 000;
- ввести возврат части пакета → начисление уменьшается;
- ввести «Выполнение плана продаж месяца, %» < 70 → предупреждение об удержании выплаты
  (порог 70% — ОТДЕЛЬНО от кредита);
- подтвердить пакет старшим тренером (кнопка «подтвердить»);
- уволить сотрудника (в `/employees`) → в карточке кнопка «Окончательный расчёт (удержать
  кредит …)» → удержание → переплата становится долгом сотрудника (в «Долги»).

**Аванс до периода:** `/payroll/employees/[id]` → блок «Авансы» → выдать аванс за текущий
месяц ДО создания периода; если расчёта нет — указать «Заработано к дате» + комментарий;
аванс управляющего с ручной базой → статус «Ожидает подтверждения регионала» → регионал
«Подтвердить и выдать». После создания периода аванс уменьшает остаток к выплате.

**Расчёт регионала:** `/payroll/regional` → создать начисление по городу (база выручка/прибыль
× %) → собственник «Утвердить» → «Выплатить из клуба» частями из клуба A и клуба B →
показывается «Выплачено по клубам», остаток, переплата; выплата сверх начисления без флажка
«оформить переплату долгом» — отклоняется.

**Выбор юрлица в выплате:** `/payroll/periods/[id]` → в форме выплаты выбрать ИП (наличные)
или ООО (безнал); одно начисление можно закрыть частью нал ИП + частью банк ООО.

**Зарплата в расходах:** после выплаты открыть `/expenses` (или аналитику) — появляется
строка расхода категории «Зарплата»; касса ООО/ИП на `/collections` уменьшается один раз;
отмена выплаты убирает строку из расходов и возвращает кассу.

**Mobile width / пустые состояния / длинные ФИО:** таблицы в `overflow-x-auto`, формы
`flex-wrap`/`grid`; пустой период — «Пока нет расчётов»; проверить на мобильной ширине.

## Покрытие 17 обязательных сценариев (Этап 6)

| # | Сценарий | Набор · проверка |
|---|---|---|
| 1 | Тренер: пакет 20 000 → 40% | `pilot-payroll-trainer` TR1 |
| 2 | Тренер: пакет 30 000 → 50% | TR2 |
| 3 | Частично проведённые занятия | TR3 |
| 4 | Возврат части пакета | TR4 |
| 5 | Кредит тренера | TR5 |
| 6 | Увольнение с остаточным кредитом | TR6, TR14 |
| 7 | Аванс 25 числа до PayrollPeriod | `pilot-payroll-preadvance` PRE1/PRE4 |
| 8 | Автоматическое связывание аванса с периодом | PRE2/PRE11 |
| 9 | Аванс не учитывается дважды | PRE3 |
| 10 | PayrollPayment как salary expense | `pilot-payroll-finance` FIN5 |
| 11 | Касса уменьшается один раз | FIN1 |
| 12 | Сторно выплаты восстанавливает кассу и P&L | FIN2, FIN9 |
| 13 | Регионал: одно начисление по городу | `pilot-payroll-regional` REG1/REG5 |
| 14 | Выплаты регионала из двух клубов | REG2/REG3 |
| 15 | Запрет превышения начисления | REG4/REG9 |
| 16 | Одно начисление, наличные ИП + банк ООО | `pilot-payroll-legalentity` LE6 |
| 17 | Tenant isolation для всех новых actions | PRE12, REG12, TR17, LE8 + resolveCalcScope/resolvePeriodScope |

**Итог обновления: READY FOR PILOT** — оба обязательных гейта закрыты (тренер доступен через
UI; PayrollPayment отражается в расходах без двойного списания), 5/5 блокеров устранены,
`pilot:full` 2766/0, оба билда зелёные, миграции аддитивные.

## Обновление: реструктуризация UX ФОТ + раздел «Авансы»

UI модуля переработан как оболочка над неизменным движком (формулы, статусы, права,
salary expense, движения кассы, долги, авансы, tenant isolation — без изменений).

- **Навигация:** горизонтальное меню (Обзор / Сотрудники и схемы / Расчётные периоды /
  Авансы / Выплаты / Долги / Регионал) на всех страницах.
- **Обзор** (`/payroll`): KPI за месяц + блок «Требуют внимания» со ссылками. Список схем
  вынесен на `/payroll/employees`.
- **Периоды:** фильтры (месяц/клуб/статус/только проблемные), цветные статусы, колонки
  Выплачено/Остаток/Проблем. Детальная страница периода — **компактный ростер** (краткий вид
  по умолчанию, подробный по кнопке); полный расчёт на отдельной странице
  `/payroll/periods/[id]/employees/[calculationId]`.
- **Авансы** (`/payroll/advances`): отдельный раздел, создание аванса **без расчётного
  периода**; переиспользует существующие `recordEmployeeAdvance/approve/cancel`. В карточке
  расчёта аванс теперь read-only summary + ссылка (форма создания убрана).
- **Выплаты** (`/payroll/payments`): движение денег отдельно от начисления; сама выплата —
  в карточке расчёта.
- **Тесты:** `npm run pilot:payroll-ux` — 19 проверок (обзор/навигация/авансы без периода/
  авто-привязка/дедуп/read-only в карточке/выплаты отдельно/краткий по умолчанию/mobile/
  старые URL/scope/закрытый период read-only). Гейты `pilot-payroll-trainer` и
  `pilot-payroll-legalentity` перенаправлены на страницу расчёта. `pilot:full` 2887/0.
- **Документация:** `docs/guides/payroll-advances-guide.md` (новая), `docs/audits/payroll-ux-audit.md`.

Инварианты §20 сохранены: аванс не дублируется при создании периода; двойного расхода нет;
наличная выплата не списывает кассу дважды; закрытый период read-only; смена схемы не
пересчитывает старый период (снапшот схемы); фильтр клуба не даёт доступ к чужому клубу;
остаток = начислено − авансы − выплаты ± сторно; один аванс на месяц (unique).
