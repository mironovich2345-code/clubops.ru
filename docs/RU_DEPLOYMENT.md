...
3. Пример ежедневного systemd timer на VM (для справки — **не** входит в код и
   ...
   > Примечание: `OnCalendar=*-*-* 00:00:00` запускает импорт вчерашнего дня сразу
   > после полуночи. Для большей надёжности можно использовать `00:05` или `00:10`
   > (`OnCalendar=*-*-* 00:05:00`), если ОФД отдаёт чеки с задержкой.

   Кроме автоимпорта, в интерфейсе есть кнопка **«Синхронизировать сейчас»**
   (Настройки → Интеграции → ОФД Такском), которая вручную подтягивает чеки за
   текущий день по активным подключениям компании. Повторный запуск безопасен —
   дубли не создаются. Кнопка не использует `CRON_SECRET` и не вызывает cron‑endpoint.
