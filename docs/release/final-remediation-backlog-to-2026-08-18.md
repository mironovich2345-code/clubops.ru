...
|---|---|---|---|---|
| REM-05 ✅ CORE DONE | Canonical `loadRecognizedExpenses` + `calculateProfit` (OFD net − recognized) + `calculateBudgetFact`; Plan/Fact + overruns + "Использовано" migrated to recognition (v2 verified + partially_paid in FULL). 31/31 real DB tests + golden scenario (profit 330,000 ₽); pilot 37/37. **CLOSED:** FIN-002, FIN-003, DATA-018/019, FIN-007 (BD-TAX). **CLOSED (via REM-05A):** FIN-001, UX-005 — all live profit readers (analytics card + KPI, dashboard club-card "Результат") now use `calculateProfit`/its exact composition; `computeManagementResult` + dead `dashboard.ts` trio deprecated (0 live callers); reader-equivalence + network reconciliation proven `test:rem-05a-profit-readers` 12/12; pilot 19/19. **PARTIAL:** FIN-006 (warn only). Remaining: production reconciliation + PostgreSQL golden (G-FIN-11/12). See `rem-05-final-report.md` + `rem-05a-final-report.md`. | FIN-001/002/003/006/007, DATA-018/019, UX-005 | **BD-03, BD-04** (ratified) | none (no schema change) | ✅ 31+12 DB tests; prod reconcile/golden pending | M |
| REM-06 ✅ CORE DONE | Split `/api/health/live` \| `/ready` (DB `SELECT 1` + `_prisma_migrations` compat + provider match + storage) \| `/dependencies`; `/api/health` kept as liveness alias; `validateDatabaseEnvironment` (prod rejects sqlite/malformed/localhost + provider mismatch) + startup fail-fast (`instrumentation.ts`); deploy/Railway/Compose gate on `/ready`; `audit:readiness` CLI. 28/28 real mock-client tests; pilot 32/32. **CLOSED:** ARCH-015, OPS-013 (startup). **PARTIAL:** OPS-003, ARCH-013/OPS-004 (real PostgreSQL DB-down/provider proof = G-READY-3..8/12, NOT EXECUTED: no PG in sandbox). See `docs/remediation/rem-06-final-report.md`. | ARCH-015, OPS-003, OPS-013, ARCH-013/OPS-004 | — | none | ✅ 28 tests; **PostgreSQL staging gate pending** | S |
| REM-07 ✅ CORE DONE | SecurityEvent table (separate from AuditLog) + allow-listed catalog + server-minted `X-Request-Id` (inbound never trusted); `recordSecurityEvent` best-effort/redacted/**fail-safe**; central integration (`requirePageAccess` + `logSecurityDenial` helper + cron); `audit:security-events` + `trace:request` CLIs; redaction (no secrets/PII/URLs). 19/19 real tests incl. failure injection; pilot 36/36. **CLOSED:** OPS-006, SEC-009. **NOT CLOSED:** ARCH-005 (observability only), SEC-002/008 (evidence only). Per-branch adoption (financial/file/scope-loader) = G-SECLOG-1/2. See `docs/remediation/rem-07-final-report.md`. | OPS-006, SEC-009 | — | additive (SecurityEvent) | ✅ 19 tests; call-site adoption + live 2-tenant gate pending | M |
| REM-08 ✅ CORE DONE | Single `payment-ledger` service (applyInvoicePaymentInTx + reversal); legacy binary `pay` RETIRED (transition blocked + "pay" button dropped) → status=paid only via a confirmed InvoicePayment; partially_paid = stored+tx-synced; `preflight:invoice-payment-ledger` + `reconcile:invoice-payments` for ledgerless/mismatch (manual, no auto-repair). 15/15 real DB tests (incl. failure injection + profit-unchanged); pilot 36/36. **CLOSED:** ARCH-010. **CLOSED for new writes / PARTIAL for historical:** DATA-005, FIN-006 (ledgerless review = G-INVLEDGER-8/9). FIN-002 stays CLOSED (REM-05). Remaining: PostgreSQL concurrency (G-INVLEDGER-10). See `docs/remediation/rem-08-final-report.md`. | ARCH-010, DATA-005, FIN-006 | — | none (ledger already exists) | ✅ 15 DB tests; PG concurrency + historical reconcile pending | S |
