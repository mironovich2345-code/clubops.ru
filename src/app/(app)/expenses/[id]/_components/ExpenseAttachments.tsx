...
export function ExpenseAttachments({
  ...
  return (
    <div className="mt-6 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 text-sm font-semibold text-slate-700">Документы</div>
      ...
      {editable ? (
        <form action={uploadAction} className="mt-4 flex flex-col gap-2 border-t border-slate-200 pt-4">
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-600">Чек/документы (JPEG, PNG, PDF; можно несколько)</span>
            <input type="file" name="documents" multiple accept="image/jpeg,image/png,application/pdf" className="block w-full text-sm" />
          </label>
