...
import { previewBulkCancelInvoices, cancelInvoicesForMonth } from "../actions";
...
function ExecButton() {
  ...
  return (
    <button type="submit" disabled={pending} className="rounded-md bg-rose-600 px-3 py-1.5 text-sm font-medium text-white shadow-sm hover:bg-rose-700 disabled:opacity-60">
      {pending ? "Отмена..." : "Отменить счета за месяц"}
    </button>
  );
}

/** Bulk-cancel active invoices for a club + month with preview and an explicit
...
export function BulkCancelInvoices({
  ...
  const [preview, previewAction] = useFormState(previewBulkCancelInvoices, initial);
  const [execState, execAction] = useFormState(cancelInvoicesForMonth, initial);
  ...
  return (
    <div className="mb-6 rounded-lg border border-rose-200 bg-white p-4 shadow-sm">
      <div className="text-sm font-semibold text-slate-700">Отменить счета за месяц</div>
      <p className="mt-0.5 text-xs text-slate-500">Массовая отмена активных счетов выбранного клуба за месяц. Отменённые счета не учитываются в отчётах.</p>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        ...
        {preview.ok ? (
          <span className="text-sm text-slate-700">
            Будет отменено: <b>{preview.count}</b> счетов · Сумма: <b>{formatKopeks(preview.totalKopeks ?? 0)}</b>
          </span>
        ) : preview.error ? (
      ...
      <form action={execAction} className="mt-3 flex flex-wrap items-end gap-3 border-t border-slate-100 pt-3">
        ...
        <label className="block">
          ...
          <input name="confirm" placeholder="ОТМЕНИТЬ" className="w-40 rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
        </label>
      ...
      {execState.done ? (
        execState.count === 0 ? (
          <p className="mt-2 text-sm text-slate-500">Под выбранные условия активных счетов не найдено.</p>
        ) : (
          <p className="mt-2 text-sm text-emerald-700">Отменено счетов: {execState.count} на сумму {formatKopeks(execState.totalKopeks ?? 0)}.</p>
        )
      ) : execState.error ? (
