  ...
  return (
    <div className="mt-4 border-t border-slate-200 pt-4 dark:border-slate-800">
      ...
      {/* Mobile: template download (full-width secondary, centered) → file field →
          upload (full-width primary), one column, equal heights. Desktop: compact. (spec §5) */}
      <div className="flex flex-col gap-3">
        <a href={`/api/sales-plans/template?month=${encodeURIComponent(month)}`} className={`${buttonClass({ variant: "secondary" })} w-full sm:w-auto`}>
          Скачать шаблон
        </a>
        <form action={previewAction} className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          <button type="submit" className={`${buttonClass({ variant: "primary" })} w-full sm:w-auto`}>Загрузить планы</button>
        </form>
      </div>
